...
from wtforms import (
    ...
    FloatField, SelectField, DateField, IntegerField
)
...
class MaterialForm(FlaskForm):
    ...
    purchase_date   = DateField('Дата закупки',    validators=[Optional()], format='%Y-%m-%d')
    delivery_date   = DateField('Дата доставки',    validators=[Optional()], format='%Y-%m-%d')
    arrival_date    = DateField('Дата прибытия',    validators=[Optional()], format='%Y-%m-%d')
    install_date    = DateField('Дата монтажа',     validators=[Optional()], format='%Y-%m-%d')
    demolition_date = DateField('Дата демонтажа',   validators=[Optional()], format='%Y-%m-%d')
...
class MaterialMovementForm(FlaskForm):
    ...
    unit = SelectField(
        ...
        choices=[
            ('', '— не выбрано —'),
            ('м', 'м'), ('м²', 'м²'), ('м³', 'м³'), ('шт', 'шт'),
            ('л', 'л'), ('кг', 'кг'), ('т', 'т'), ('пог.м', 'пог.м'),
            ('упак', 'упак'), ('комплект', 'комплект')
        ],
        validators=[Optional()]
    )
...
class ScheduleWorkForm(FlaskForm):
    ...
    name = TextAreaField(
        'Название работы',
        validators=[DataRequired(message='Введите название работы')]
    )
