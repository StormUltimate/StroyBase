...
material_documents = db.Table(
    ...
    db.Column(
        "material_id",
        db.Integer,
        db.ForeignKey("floor_materials.id", ondelete="CASCADE"),
        primary_key=True,
    ),
    db.Column(
        "document_id",
    ...
    db.Column(
        "doc_type", db.String(50), nullable=True
    ),  # certificate, invoice, photo, act, other...
...
class Project(db.Model):
    ...
    buildings = db.relationship(
        "Building", backref="project", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class Building(db.Model):
    ...
    floors = db.relationship(
        "Floor", backref="building", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class BuildingParticipant(db.Model):
    ...
    building_id = db.Column(
        db.Integer, db.ForeignKey("buildings.id", ondelete="CASCADE"), nullable=True
    )
    role_name = db.Column(
        db.String(100), nullable=True
    )  # Заказчик, Генподрядчик, Технадзор, Застройщик, ООО
...
class ScheduleWork(db.Model):
    ...
    floor_id = db.Column(
        db.Integer, db.ForeignKey("floors.id", ondelete="SET NULL"), nullable=True
    )
    name = db.Column(
        db.Text, nullable=True
    )  # полностью свободное название работы от пользователя
    ...
    )  # "Запланировано", "В работе", "Завершено", "Просрочено"
    notes = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=True)

    project = db.relationship(
        "Project",
        backref=db.backref("schedule_works", lazy=True, cascade="all, delete-orphan"),
    )
...
class PlanTask(db.Model):
    ...
    task_type = db.Column(
        db.String(50), nullable=True
    )  # Подготовка, Демонтаж, Монтаж — для цвета/секции в Gantt
    dependency_id = db.Column(
        db.Integer, db.ForeignKey("plan_tasks.id", ondelete="SET NULL"), nullable=True
    )
    building_id = db.Column(
        db.Integer, db.ForeignKey("buildings.id", ondelete="SET NULL"), nullable=True
    )
    floor_id = db.Column(
        db.Integer, db.ForeignKey("floors.id", ondelete="SET NULL"), nullable=True
    )
    ...
    project = db.relationship(
        "Project",
        backref=db.backref("plan_tasks", lazy=True, cascade="all, delete-orphan"),
    )
...
class Work(db.Model):
    ...
    percent_complete = db.Column(db.Float, nullable=True)  # 0.0 - 100.0
    initial_executed = db.Column(
        db.Float, nullable=True
    )  # объём, учтённый до ежедневного учёта (импорт/ручной ввод); итог = initial_executed + sum(work_progress.daily_execution)
    ...
    project = db.relationship(
        "Project", backref=db.backref("works", lazy=True, cascade="all, delete-orphan")
    )
    building = db.relationship(
        "Building", backref=db.backref("works", lazy=True, cascade="all, delete-orphan")
    )
...
class WorkProgress(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    work_id = db.Column(
        db.Integer, db.ForeignKey("works.id", ondelete="CASCADE"), nullable=True
    )
    ...
    work = db.relationship(
        "Work",
        backref=db.backref("progress", lazy="dynamic", cascade="all, delete-orphan"),
    )
...
class DailyWorkforce(db.Model):
    ...
    workers_count_night = db.Column(
        db.Integer, nullable=True
    )  # человек в ночной смене (отдельно от дня)
    shift_hours = db.Column(db.Float, nullable=True)  # дневная смена, часов (макс. 12)
    shift_hours_night = db.Column(
        db.Float, nullable=True
    )  # ночная смена, часов (макс. 12), всего в сутках макс. 24
    notes = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=True)

    project = db.relationship(
        "Project",
        backref=db.backref("daily_workforce", lazy=True, cascade="all, delete-orphan"),
    )
...
class WorkPerformer(db.Model):
    ...
    company = db.Column(
        db.String(255), nullable=True
    )  # в API отдаётся как organization
    notes = db.Column(db.Text, nullable=True)  # в API отдаётся как note
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=True)

    project = db.relationship(
        "Project",
        backref=db.backref("work_performers", lazy=True, cascade="all, delete-orphan"),
    )
...
class Floor(db.Model):
    ...
    plans = db.relationship(
        "Plan", backref="floor", lazy="dynamic", cascade="all, delete-orphan"
    )
    marks = db.relationship(
        "Mark", backref="floor", lazy="dynamic", cascade="all, delete-orphan"
    )
    materials = db.relationship(
        "FloorMaterial", backref="floor", lazy="dynamic", cascade="all, delete-orphan"
    )
    equipment = db.relationship(
        "FloorEquipment", backref="floor", lazy="dynamic", cascade="all, delete-orphan"
    )
    quantities = db.relationship(
        "FloorQuantity", backref="floor", lazy="dynamic", cascade="all, delete-orphan"
    )
    tasks = db.relationship(
        "Task", backref="floor", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class Document(db.Model):
    ...
    file_modified_at = db.Column(
        db.DateTime, nullable=True
    )  # дата создания/изменения файла (или вручную)
    ...
    work_types = db.relationship(
        "DocType", secondary=document_work_types, backref="documents"
    )
    linked_materials = db.relationship(
        "FloorMaterial",
        secondary=material_documents,
        backref="documents",
        lazy="dynamic",
    )
    orders = db.relationship(
        "Order", secondary=order_documents, backref="documents", lazy="dynamic"
    )
...
class Plan(db.Model):
    ...
    marks = db.relationship(
        "Mark", backref="plan", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class MaterialCategory(db.Model):
    ...
    parent_id = db.Column(
        db.Integer, db.ForeignKey("material_categories.id"), nullable=True
    )
...
class FloorMaterial(db.Model):
    ...
    orders = db.relationship(
        "Order", backref="material", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class FloorEquipment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    floor_id = db.Column(
        db.Integer, db.ForeignKey("floors.id", ondelete="CASCADE"), nullable=True
    )
    name = db.Column(db.String(255), nullable=True)
    brand = db.Column(db.String(255), nullable=True)
    passport_document_id = db.Column(
...
class FloorQuantity(db.Model):
    ...
    name = db.Column(
        db.String(255), nullable=True
    )  # например: Площадь потолка, Плинтус, Шпатлёвка стен
...
class Order(db.Model):
    ...
    material_id = db.Column(
        db.Integer, db.ForeignKey("floor_materials.id"), nullable=False
    )
...
class MaterialMovement(db.Model):
    ...
    project = db.relationship(
        "Project", backref=db.backref("material_movements", lazy=True)
    )
    building = db.relationship(
        "Building", backref=db.backref("material_movements", lazy=True)
    )
    floor = db.relationship(
        "Floor", backref=db.backref("material_movements", lazy=True)
    )
...
class MovementDocument(db.Model):
    ...
    movement_id = db.Column(
        db.Integer,
        db.ForeignKey("material_movements.id", ondelete="CASCADE"),
        primary_key=True,
    )
    document_id = db.Column(
        db.Integer, db.ForeignKey("documents.id", ondelete="CASCADE"), primary_key=True
    )
    doc_type = db.Column(
        db.String(50), nullable=True
    )  # invoice, upd, certificate, other
    ...
    movement = db.relationship(
        "MaterialMovement",
        backref=db.backref(
            "movement_document_links", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
    document = db.relationship(
        "Document", backref=db.backref("movement_document_links", lazy="dynamic")
    )
...
class User(UserMixin, db.Model):
    ...
    role = db.Column(
        db.String(20), default="viewer"
    )  # viewer, editor, pto, foreman, admin
    is_active = db.Column(db.Boolean, default=True)
