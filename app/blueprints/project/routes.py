...
def dashboard(project_id):
    ...
    if from_date_str:
        try:
            ...
            query = query.filter(
                Document.file_modified_at.isnot(None),
    ...
    project_floors = (
        Floor.query.join(Building)
        .filter(Building.project_id == project_id)
        .order_by(Building.name, Floor.name)
        .all()
    )
    ...
    try:
        ...
        project_schedule_works = sw_query.order_by(
            ScheduleWork.planned_start.asc().nullslast(), ScheduleWork.id
        ).all()
    except ProgrammingError:
        db.session.rollback()
    ...
    try:
        daily_workforce = (
            DailyWorkforce.query.filter_by(project_id=project_id)
            .order_by(DailyWorkforce.date.desc().nullslast())
            .all()
        )
    except Exception:
        db.session.rollback()
    ...
    for r in daily_workforce:
        if r.date:
            ...
            daily_totals[r.date]["hours"] += (r.shift_hours or 0.0) + (
                getattr(r, "shift_hours_night", None) or 0.0
            )

    # Справочник исполнителей работ (таблица может отсутствовать)
    try:
        work_performers = (
            WorkPerformer.query.filter_by(project_id=project_id)
            .order_by(WorkPerformer.name)
            .all()
        )
    except Exception:
        db.session.rollback()
    ...
    for day in range(1, month_days + 1):
        ...
        th = sum(
            (x.shift_hours or 0.0) + (getattr(x, "shift_hours_night", None) or 0.0)
            for x in recs
        )
        ...
        calendar_days.append(
            {
                ...
                "total_hours": th,
                "records": recs,
                "total_workers_day": tw_day,
                "total_workers_night": tw_night,
                "total_hours_day": th_day,
                "total_hours_night": th_night,
            }
        )
    ...
    total_floors_area = (
        sum((f.total_area_m2 or 0) for f in project_floors) if project_floors else 0
    )

    try:
        project_movements = (
            MaterialMovement.query.filter_by(project_id=project_id)
            .order_by(
    ...
    try:
        plan_tasks = (
            PlanTask.query.filter_by(project_id=project_id)
            .order_by(
    ...
    works_by_building, works_summary_project, works_summary_buildings = (
        _build_works_summary(
            ...
            buildings,
            graphs_building_id,
            graphs_works_from_str,
            graphs_works_to_str,
        )
    )
    works_aggregated_by_name = _build_works_aggregated_by_name(
        project_id, graphs_works_from_str, graphs_works_to_str
    )
    ...
    return render_template(
        ...
        doc_docs=doc_docs,
        project_schedule_works=project_schedule_works,
...
def _build_works_summary_fallback(
    project_id,
    buildings,
    graphs_building_id=None,
    graphs_works_from_str=None,
    ...
    try:
        from_date_works = (
            _parse_date_project(graphs_works_from_str)
            if graphs_works_from_str
            else None
        )
        to_date_works = (
            _parse_date_project(graphs_works_to_str) if graphs_works_to_str else None
        )
        sql_works = (
        ...
        if work_ids:
            t = text(
                "SELECT work_id, date, daily_execution FROM work_progress WHERE work_id IN :ids"
            ).bindparams(bindparam("ids", expanding=True))
            prog = db.session.execute(t, {"ids": work_ids}).fetchall()
            for work_id, pdate, daily in prog:
                if (
                    pdate
                    and (from_date_works is None or pdate >= from_date_works)
                    and (to_date_works is None or pdate <= to_date_works)
                ):
        ...
        pct_list = [
            e["percent"] for e in entries_with_volume if e.get("percent") is not None
        ]
        project_avg_percent = (
            round(sum(pct_list) / len(pct_list), 1) if pct_list else None
        )
        ...
        for b in buildings:
            entries = by_building.get(b.id, [])
            entries_vol = [
                e for e in entries if (e["volume"] is not None and e["volume"] > 0)
            ]
            ...
            building_avg_percent = (
                round(sum(pct_list_b) / len(pct_list_b), 1) if pct_list_b else None
            )
    ...
    try:
        ...
        def _executed_for_work(work):
            ...
            for p in work.progress:
                if (
                    p.date
                    and (from_date_works is None or p.date >= from_date_works)
                    and (to_date_works is None or p.date <= to_date_works)
                ):
        ...
        db.session.rollback()
        return _build_works_summary_fallback(
            project_id,
            buildings,
            graphs_building_id,
            graphs_works_from_str,
...
def _build_works_aggregated_by_name_fallback(
    project_id, graphs_works_from_str=None, graphs_works_to_str=None
):
    ...
    try:
        ...
        rows = db.session.execute(
            text(
                "SELECT id, name, volume, unit, category, system_subsection FROM works WHERE project_id = :pid ORDER BY name, id"
            ),
            {"pid": project_id},
        ).fetchall()
        work_ids = [r[0] for r in rows]
...
def _build_works_aggregated_by_name(
    project_id, graphs_works_from_str=None, graphs_works_to_str=None
):
    ...
    result = []
    try:
        works_query = Work.query.filter_by(project_id=project_id).order_by(
            Work.name, Work.id
        )
        all_works = works_query.all()
        from_date_works = (
        ...
        for w in all_works:
            ...
            if category == "Инженерные системы":
                subsection = (
                    getattr(w, "system_subsection_or_default", None)
                    or getattr(w, "system_subsection", None)
                    or "Прочие системы"
                ).strip()
            key = (category, subsection, name_key)
            if key not in by_name:
        ...
        db.session.rollback()
        return _build_works_aggregated_by_name_fallback(
            project_id, graphs_works_from_str, graphs_works_to_str
        )
    return result
...
def works_summary_json(project_id):
    ...
    return jsonify(
        works_summary_project=out_project, works_summary_buildings=out_buildings
    )
    ...
    for t in plan_tasks:
        ...
        if t.start_date and t.end_date and t.end_date >= t.start_date:
            ...
            if t.dependency_id and t.dependency_id in id_map:
                lines_here.append(
                    '    "%s" :%s, after %s, %dd'
                    % (name_esc, id_map[t.id], id_map[t.dependency_id], dur_days)
                )
            else:
                lines_here.append(
                    '    "%s" :%s, %s, %dd'
                    % (name_esc, id_map[t.id], t.start_date.isoformat(), dur_days)
                )
        else:
...
def _work_daily_json(project_id, work_id, keys=("dates", "executions")):
    ...
    try:
        q = WorkProgress.query.filter_by(work_id=work_id).order_by(
            WorkProgress.date.asc()
        )
        if from_d:
    ...
    try:
        ...
        if os.path.isdir(os.path.join(root, "scripts")):
            ...
            script_path = os.path.join(
                os.path.dirname(root), "scripts", "export_works_to_excel.py"
            )
        ...
        return send_file(
            data,
            as_attachment=True,
            download_name=f"works_project_{project_id}.xlsx",
...
def works_import_excel(project_id):
    ...
    try:
        ...
        if os.path.isdir(os.path.join(root, "scripts")):
            ...
            script_path = os.path.join(
                os.path.dirname(root), "scripts", "import_works_from_excel.py"
            )
        if not os.path.isfile(script_path):
...
@project_bp.route(
    "/<int:project_id>/works/<int:work_id>/update_daily", methods=["POST"]
)
@login_required
def update_daily(project_id, work_id):
    ...
    wants_json = (
        request.is_json
        or (request.get_json(silent=True) is not None)
    ...
    try:
        ...
        row = db.session.execute(
            text(
                "SELECT id, project_id, building_id, name, volume FROM works WHERE id = :id AND project_id = :pid"
            ),
            {"id": work_id, "pid": project_id},
        ).fetchone()
        if not row:
    ...
    date_str = request.form.get("date") or (request.get_json(silent=True) or {}).get(
        "date"
    )
    value_raw = request.form.get("value") or (request.get_json(silent=True) or {}).get(
        "value"
    )
    ...
    if val < 0:
        if wants_json:
            return (
                jsonify({"ok": False, "error": "Значение не может быть отрицательным"}),
                400,
            )
    ...
    try:
        ...
        if wants_json:
            ...
            if hasattr(work, "progress"):
                ...
                daily_sum = (
                    db.session.execute(
                        text(
                            "SELECT COALESCE(SUM(daily_execution), 0) FROM work_progress WHERE work_id = :wid"
                        ),
                        {"wid": work_id},
                    ).scalar()
                    or 0
            ...
            new_pct = (
                round(new_total / vol * 100.0, 1) if vol and float(vol) > 0 else None
            )
            return jsonify(
                {
                    "ok": True,
        ...
        flash(
            "Ошибка сохранения: "
            + _format_db_programming_error(_extract_db_programming_error(e)),
            "danger",
        )
    return redirect(
        url_for("project.dashboard", project_id=project_id) + "#project-graphs"
...
def add_workforce(project_id):
    ...
    try:
        ...
        db.session.rollback()
        flash(
            "Таблица daily_workforce не найдена в БД. Структура БД устарела. Запустите миграции: flask db upgrade.",
            "warning",
        )
    cal_month = request.form.get("calendar_month", type=int)
    cal_year = request.form.get("calendar_year", type=int)
    q = (
        {"calendar_month": cal_month, "calendar_year": cal_year}
        if cal_month and cal_year
        else {}
    )
    return redirect(
        url_for("project.dashboard", project_id=project_id, **q) + "#project-people"
    )
...
def edit_workforce(project_id, record_id):
    project = Project.query.get_or_404(project_id)
    record = DailyWorkforce.query.filter_by(
...
@project_bp.route(
    "/<int:project_id>/workforce/<int:record_id>/delete", methods=["POST"]
)
@login_required
def delete_workforce(project_id, record_id):
    ...
    return redirect(
        ref
        if ref
        else url_for("project.dashboard", project_id=project_id) + "#project-people"
    )
...
def workforce_day(project_id, date_str):
    ...
    try:
        records = DailyWorkforce.query.filter_by(
    ...
    out_records = [
        {
            ...
            "shift_hours_night": (
                float(r.shift_hours_night)
                if getattr(r, "shift_hours_night", None) is not None
                else 0.0
            ),
            "notes": r.notes or "",
        }
    ...
    out_performers = [
        {"name": p.name or "", "organization": p.company or ""} for p in performers
    ]
...
def workforce_day_save(project_id):
    ...
    if not day_date:
        ...
        return redirect(
            url_for("project.dashboard", project_id=project_id) + "#project-people"
        )
    ...
    try:
        ...
        for rec in records_data:
            ...
            if workers_count is not None and (
                not isinstance(workers_count, int) or workers_count < 0
            ):
                workers_count = 0
            elif workers_count is None:
                workers_count = 0
            workers_count_night = rec.get("workers_count_night")
            if workers_count_night is not None and (
                not isinstance(workers_count_night, int) or workers_count_night < 0
            ):
        ...
        db.session.rollback()
        flash(
            "Ошибка сохранения. Проверьте наличие таблицы daily_workforce в БД.",
            "warning",
        )
    calendar_month = (
        request.args.get("month")
        or data.get("month")
        or request.form.get("calendar_month", type=int)
    )
    calendar_year = (
        request.args.get("year")
        or data.get("year")
        or request.form.get("calendar_year", type=int)
    )
    today = date.today()
...
def add_performer(project_id):
    ...
    try:
        performer = WorkPerformer(
            project_id=project_id, name=name, company=company, notes=notes
        )
        ...
        db.session.rollback()
        flash(
            "Таблица work_perформers не найдена. Структура БД устарела. Запустите миграции: flask db upgrade.",
            "warning",
        )
    cal_month = request.form.get("calendar_month", type=int)
    cal_year = request.form.get("calendar_year", type=int)
...
@project_bp.route(
    "/<int:project_id>/performers/<int:performer_id>/delete", methods=["POST"]
)
@login_required
def delete_performer(project_id, performer_id):
    project = Project.query.get_or_404(project_id)
    performer = WorkPerformer.query.filter_by(
        id=performer_id, project_id=project_id
    ).first_or_404()
    try:
...
def upload_floor_plan(floor_id):
    ...
    rel_image_path = os.path.join(
        "media", "plans", str(floor.id), image_filename
    ).replace("\\", "/")
...
def create_building(project_id):
    ...
    return render_template(
        "project/create_building.html", project=project
    )  # Шаблон для формы
...
def upload_documents(project_id):
    ...
    for file in files:
        ...
        rel_stored = os.path.join(
            "media", "projects", str(project_id), sub_dir, unique_fs_filename
        ).replace("\\", "/")
        rel_thumbnail = (
            thumbnail_path
            and os.path.join(
                "media", "projects", str(project_id), sub_dir, thumb_filename
            ).replace("\\", "/")
            or None
        )
            ...
            for wt_id in all_type_ids:
                db.session.execute(
                    document_work_types.insert().values(
                        document_id=document.id, doc_type_id=wt_id
                    )
                )
    ...
    return redirect(
        url_for("project.dashboard", project_id=project_id, **redirect_params)
    )
...
def edit_document(doc_id):
    ...
    if request.method == "POST":
        ...
        new_type_ids = [
            int(x) for x in request.form.getlist("work_type_ids") if x.isdigit()
        ]
        extra_tag_names = _parse_tags_raw(request.form.get("tags"))
        extra_type_ids = (
            _ensure_doc_types_by_names(extra_tag_names) if extra_tag_names else []
    ...
    back_url = (
        request.referrer
        or url_for("project.dashboard", project_id=project.id) + "#docs"
    )
    return render_template(
