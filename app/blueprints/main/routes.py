...
def dashboards():
    ...
    people_stats = {}
    try:
        expr_hours = func.coalesce(DailyWorkforce.shift_hours, 0) + func.coalesce(
            DailyWorkforce.shift_hours_night, 0
        )
        rows = (
            db.session.query(
                DailyWorkforce.project_id,
                func.coalesce(func.sum(DailyWorkforce.workers_count), 0).label(
                    "total_workers"
                ),
                func.coalesce(func.sum(expr_hours), 0).label("total_hours"),
            )
            .filter(DailyWorkforce.project_id.isnot(None))
            .group_by(DailyWorkforce.project_id)
            .all()
        )
        for r in rows:
            people_stats[r.project_id] = {
                "total_workers": int(r.total_workers or 0),
        ...
        try:
            rows = (
                db.session.query(
                    ...
                    func.coalesce(func.sum(DailyWorkforce.shift_hours), 0).label(
                        "total_hours"
                    ),
                )
                .filter(DailyWorkforce.project_id.isnot(None))
        ...
        if work_ids:
            rows = (
                db.session.query(
                    ...
                    func.coalesce(func.sum(WorkProgress.daily_execution), 0).label(
                        "sum_exec"
                    ),
                )
        ...
        try:
            rows = (
                db.session.query(
                    ...
                    func.coalesce(
                        func.sum(DailyWorkforce.workers_count_night), 0
                    ).label("w_night"),
                )
                .filter(
...
def create_project():
    ...
    if form.validate_on_submit():
        ...
        media_path = os.path.join(
            current_app.config["MEDIA_DIR"], "projects", str(project.id)
        )
...
def _project_delete_impact(project_id):
    ...
    floors_count = (
        Floor.query.join(Building).filter(Building.project_id == project_id).count()
    )
...
def delete_project(project_id):
    ...
    try:
        movement_ids = [
            m.id for m in MaterialMovement.query.filter_by(project_id=project_id).all()
        ]
        ...
        db.session.commit()
        media_path = os.path.join(
            current_app.config["MEDIA_DIR"], "projects", str(project_id)
        )
        if os.path.exists(media_path):
            shutil.rmtree(media_path)
        current_app.logger.info(
            'Project id=%s "%s" deleted (cascade: buildings, floors, documents, movements)',
            project_id,
            project_name,
        )
