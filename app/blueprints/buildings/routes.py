...
from flask import Blueprint, render_template, request, flash, redirect, url_for, current_app, g, send_from_directory, abort
from flask_login import login_required, current_user
from sqlalchemy import or_
from sqlalchemy.exc import ProgrammingError
from app.extensions import db
from app.models import Building, Project, Floor, Document, DocType, document_work_types, ScheduleWork, FloorMaterial, MaterialCategory, FloorQuantity
...
def view_building(building_id):
    ...
    if contract_doc_type_id:
        c_query = Document.query.filter_by(building_id=building_id).join(
            document_work_types
        ...
        if contract_date_from:
            try:
                c_query = c_query.filter(
                    Document.contract_date >= datetime.strptime(contract_date_from, '%Y-%m-%d').date()
                )
            except ValueError:
                pass
        if contract_date_to:
            try:
                c_query = c_query.filter(
                    Document.contract_date <= datetime.strptime(contract_date_to, '%Y-%m-%d').date()
                )
            except ValueError:
                pass
        if contract_search:
        ...
        contract_docs = c_query.order_by(
            Document.contract_date.desc().nullslast(),
            Document.uploaded_at.desc()
        ).all()
    ...
    try:
        ...
        works_summary_buildings = [{'building': building, 'total_volume': 0.0, 'total_executed': 0.0, 'percent': None, 'building_avg_percent': None}]
...
def add_schedule_work(building_id):
    ...
    return redirect(url_for('buildings.view_building', building_id=building_id) + '#schedules')
...
def upload_building_documents(building_id):
    ...
    work_type_ids = [int(wt) for wt in request.form.getlist('work_type_ids') if wt.isdigit()]
    ...
    for file in files:
        ...
        rel_path = os.path.join('media', 'projects', str(project_id), sub_dir, unique_name).replace('\\', '/')
        rel_thumb = None
        if thumbnail_path:
            rel_thumb = os.path.join('media', 'projects', str(project_id), sub_dir, f"thumb_{unique_name}").replace('\\', '/')
...
def delete_document(doc_id):
    ...
    flash(f'Document "{filename}" removed from database (physical file preserved)', 'info')
    return redirect(url_for('buildings.view_building', building_id=building_id))
