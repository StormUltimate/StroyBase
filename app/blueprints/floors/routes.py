...
def _floor_document_choices(floor):
    ...
    docs = (
        Document.query.filter_by(floor_id=floor.id)
        .order_by(Document.uploaded_at.desc())
        .all()
    )
    return [("", "— не выбрано —")] + [
        (str(d.id), (d.title or d.filename or f"Документ #{d.id}")[:80]) for d in docs
    ]
...
def create_floor(building_id):
    ...
    if request.method == "POST":
        ...
        floor = Floor(building_id=building_id, name=name, floor_type=floor_type)
...
def edit_floor(floor_id):
    ...
    if request.method == "POST":
        ...
        try:
            area_m2 = (
                float(request.form.get("area_m2"))
                if request.form.get("area_m2")
                else None
            )
        ...
        try:
            total_area_m2 = (
                float(request.form.get("total_area_m2"))
                if request.form.get("total_area_m2")
                else None
            )
        except (ValueError, TypeError):
            total_area_m2 = None
        ...
        return redirect(
            url_for("buildings.view_building", building_id=floor.building_id)
        )
...
def view_floor(floor_id):
        ...
        if uploaded_to:
            try:
                to_dt = datetime.strptime(
                    uploaded_to + " 23:59:59", "%Y-%m-%d %H:%M:%S"
                )
    ...
    plans = (
        Plan.query.filter_by(floor_id=floor_id).order_by(Plan.uploaded_at.desc()).all()
    )
    try:
    ...
    try:
        floor_movements = (
            MaterialMovement.query.filter_by(floor_id=floor_id)
            .order_by(
            ...
            for fm in floor_materials:
                ...
                if name_ok:
                    if fm.planned_quantity is not None:
                        total = (total or Decimal("0")) + Decimal(
                            str(fm.planned_quantity)
                        )
    ...
    try:
        floor_materials = (
            FloorMaterial.query.filter_by(floor_id=floor_id)
            .order_by(FloorMaterial.name)
            .all()
        )

        # Фактический объём по ключу (наименование, бренд, ед. изм.)
        fact_by_key = {
            key: row["quantity_total"] for key, row in material_summary_map.items()
        }
...
def upload_documents(floor_id):
    ...
    upload_dir = os.path.join(
        ...
        "media",
        "projects",
        str(floor.building.project_id),
        f"building_{floor.building_id}",
        f"floor_{floor_id}",
    )
    ...
    for file in files:
        ...
        rel_path = (
            file_path.replace(os.path.join(current_app.root_path, "static"), "")
            .replace("\\", "/")
            .lstrip("/")
        )
        ...
        all_type_ids = (
            sorted({int(wt) for wt in work_type_ids} | set(extra_type_ids))
            if work_type_ids or extra_type_ids
            else []
        )
        for wt_id in all_type_ids:
            stmt = document_work_types.insert().values(
...
def upload_floor_document(floor_id):
    ...
    if request.method == "POST":
        ...
        flash(
            "Документ загружен. Выберите его в списке сертификата/паспорта.", "success"
        )
        ...
        equipment_id = request.form.get("equipment_id") or request.args.get(
            "equipment_id"
        )
        if return_to == "edit_material" and material_id:
            return redirect(
                url_for(
                    "floors.edit_material",
                    floor_id=floor_id,
...
def create_material(floor_id):
    ...
    form.category_id.choices = [("", "— не выбрано —")] + [
        (str(c.id), c.name)
        for c in MaterialCategory.query.order_by(MaterialCategory.name).all()
    ]
    ...
    return render_template(
        "floors/create_material.html", form=form, floor=floor, title="Добавить материал"
    )


@bp.route(
    "/floor/<int:floor_id>/edit_material/<int:material_id>", methods=["GET", "POST"]
)
@login_required
def edit_material(floor_id, material_id):
    ...
    if form.validate_on_submit():
        ...
        material.certificate_document_id = (
            int(form.certificate_document_id.data)
            if form.certificate_document_id.data
            else None
        )
        material.category_id = (
            int(form.category_id.data) if form.category_id.data else None
        )
        db.session.commit()
...
def create_quantity(floor_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for("floors.view_floor", floor_id=floor_id) + "#tech")
    return render_template(
        "floors/create_quantity.html",
        form=form,
        floor=floor,
        title="Добавить площадь / величину",
    )


@bp.route(
    "/floor/<int:floor_id>/edit_quantity/<int:quantity_id>", methods=["GET", "POST"]
)
@login_required
def edit_quantity(floor_id, quantity_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for("floors.view_floor", floor_id=floor_id) + "#tech")
    return render_template(
        "floors/edit_quantity.html",
        form=form,
        floor=floor,
        quantity=qty,
        title="Редактировать величину",
    )
...
@bp.route(
    "/floor/<int:floor_id>/edit_equipment/<int:equipment_id>", methods=["GET", "POST"]
)
@login_required
...
def floor_plan(floor_id, plan_id):
    ...
    docs_query = Document.query.filter(
        Document.floor_id == floor_id, Document.mark_id.isnot(None)
    )
    ...
    if work_types_str:
        ...
        if work_type_ids:
            docs_query = (
                docs_query.join(document_work_types)
                .join(DocType)
                .filter(DocType.id.in_(work_type_ids))
            )
...
def add_mark(floor_id, plan_id):
    ...
    try:
        ...
        flash(
            "Не удалось определить координаты метки. Повторите выбор точки на плане.",
            "warning",
        )
        return redirect(
            url_for("floors.floor_plan", floor_id=floor_id, plan_id=plan_id)
        )

    note = request.form.get("note") or None

    mark = Mark(plan_id=plan.id, floor_id=floor.id, x=x, y=y, note=note)
    db.session.add(mark)
    db.session.commit()
...
def upload_floor_plan(floor_id):
    ...
    plan_dir = os.path.join(
        current_app.root_path, "static", "media", "plans", str(floor_id)
    )
    ...
    if filename.lower().endswith(".pdf"):
        try:
            ...
            image_filename = (
                f"{os.path.splitext(unique_filename)[0]}_page_{page_number}.png"
            )
    ...
    rel_image_path = (
        image_path.replace(os.path.join(current_app.root_path, "static"), "")
        .replace("\\", "/")
        .lstrip("/")
    )
    ...
    return redirect(
        url_for("floors.floor_plan", floor_id=floor_id, plan_id=new_plan.id)
    )
...
def select_mark_for_photo(photo_id):
    ...
    floor = Floor.query.get_or_404(doc.floor_id)
    plans = (
        Plan.query.filter_by(floor_id=floor.id).order_by(Plan.uploaded_at.desc()).all()
    )
    if not plans:
        flash(
            "Для этого этажа ещё не загружены планы. Сначала загрузите план этажа.",
            "warning",
        )
        return redirect(url_for("floors.edit_photo", photo_id=photo_id))
...
def bind_mark_to_photo(photo_id):
    ...
    try:
        ...
        return redirect(
            url_for("floors.select_mark_for_photo", photo_id=photo_id, plan_id=plan.id)
        )
