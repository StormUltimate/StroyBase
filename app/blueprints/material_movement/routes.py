...
from app.models import (
    Document, Floor, Building, Project,
    MaterialMovement, MovementDocument, FloorMaterial
)
...
def _build_project_stock_summary(project_id):
    ...
    try:
        ...
        db.session.rollback()
        return [], 'Таблица material_movements не найдена в БД. Создайте её в pgAdmin (см. комментарий в models.py).'
    ...
    result.sort(key=lambda r: ((r["material_name"] or '').lower(), (r["brand"] or '').lower() if r["brand"] else '', r["unit"] or ''))
    return result, None
...
def api_buildings():
    ...
    try:
        rows = Building.query.filter_by(project_id=project_id).order_by(Building.name).all()
        items = []
        for b in rows:
            fallback = f'Корпус {b.id}'
            name = _sanitize_display_name(b.name, fallback)
            if name == fallback and b.name and str(b.name).strip():
                current_app.logger.debug('material_movement api_buildings: sanitization fallback for building id=%s raw_name=%r', b.id, b.name[:100] if b.name else None)
    ...
    try:
        ...
        for f in rows:
            ...
            if name == fallback and f.name and str(f.name).strip():
                current_app.logger.debug('material_movement api_floors: sanitization fallback for floor id=%s raw_name=%r', f.id, f.name[:100] if f.name else None)
...
def index():
    ...
    try:
        ...
        if material_name:
            query = query.filter(MaterialMovement.material_name.ilike(f'%{material_name}%'))
        if date_from:
            try:
                query = query.filter(MaterialMovement.movement_date >= datetime.strptime(date_from, '%Y-%m-%d').date())
            except ValueError:
                pass
        if date_to:
            try:
                query = query.filter(MaterialMovement.movement_date <= datetime.strptime(date_to, '%Y-%m-%d').date())
            except ValueError:
                pass
        if movement_type:
            query = query.filter(MaterialMovement.movement_type == movement_type)

        query = query.order_by(MaterialMovement.movement_date.desc().nullslast(), MaterialMovement.created_at.desc())
        ...
        movements = []
        pagination = None
        flash('Таблица material_movements не найдена в БД. Создайте её в pgAdmin (скрипт sql_material_movements.sql в корне проекта).', 'warning')
    ...
    projects_display = [(p.id, _sanitize_display_name(p.name, f'Проект {p.id}')) for p in projects]
...
def create_movement():
    ...
    projects = Project.query.order_by(Project.name).all()
    form.project_id.choices = [('', '— Выберите проект —')] + [(p.id, p.name) for p in projects]
    project_id = request.form.get('project_id', type=int)
    form.building_id.choices = [('', '— Не привязан —')]
    form.floor_id.choices = [('', '— Не привязан —')]
    if project_id:
        form.building_id.choices += [(b.id, b.name) for b in Building.query.filter_by(project_id=project_id).order_by(Building.name).all()]
    building_id = request.form.get('building_id', type=int)
    if building_id:
        form.floor_id.choices += [(f.id, f.name) for f in Floor.query.filter_by(building_id=building_id).order_by(Floor.name).all()]

    if not form.validate_on_submit():
        for field, errors in form.errors.items():
            for e in errors:
                flash(f'{getattr(form, field).label.text}: {e}', 'danger')
        return redirect(url_for('movement.index', **{k: v for k, v in request.args.items() if v}))
    ...
    try:
        ...
        db.session.rollback()
        flash('Ошибка базы данных. Убедитесь, что таблица material_movements создана (см. комментарий в models.py).', 'danger')
    except Exception as e:
        db.session.rollback()
        flash(f'Ошибка при сохранении: {e}', 'danger')

    return redirect(url_for('movement.index', project_id=form.project_id.data, **{k: v for k, v in request.args.items() if k != 'project_id' and v}))
...
def edit_movement(movement_id):
    ...
    projects = Project.query.order_by(Project.name).all()
    form.project_id.choices = [('', '— Выберите проект —')] + [(p.id, p.name) for p in projects]
    project_id = movement.project_id if request.method == 'GET' else request.form.get('project_id', type=int)
    building_id = movement.building_id if request.method == 'GET' else request.form.get('building_id', type=int)
    form.building_id.choices = [('', '— Не привязан —')]
    form.floor_id.choices = [('', '— Не привязан —')]
    if project_id:
        form.building_id.choices += [(b.id, b.name) for b in Building.query.filter_by(project_id=project_id).order_by(Building.name).all()]
    ...
    try:
        document_links = movement.movement_document_links.order_by(MovementDocument.order).all()
    except Exception:
        document_links = []
...
DOC_TYPE_LABELS = {'invoice': 'Накладная', 'upd': 'УПД', 'certificate': 'Сертификат', 'other': 'Другое'}
...
def attach_movement_document(movement_id):
    ...
    upload_dir = os.path.join(
        current_app.root_path, 'static', 'media', 'projects', str(movement.project_id),
        'movement_docs', str(movement_id)
    )
    ...
    doc = Document(
        ...
        title=request.form.get('title') or f"{DOC_TYPE_LABELS.get(doc_type)} — {movement.material_name or 'материал'}",
        filename=filename,
    ...
    db.session.add(doc)
    db.session.flush()
    max_order = db.session.query(func.max(MovementDocument.order)).filter(
        MovementDocument.movement_id == movement_id
    ).scalar() or 0
...
def serve_movement_document(movement_id, document_id):
    ...
    if not doc or not doc.stored_path:
        ...
        abort(404)
    full_path = os.path.join(current_app.root_path, 'static', doc.stored_path.replace('/', os.sep))
