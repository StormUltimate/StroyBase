...
def create_app():
    ...
    def require_login():
        ...
        if not current_user.is_authenticated:
            flash('Для работы с объектами, фото, видео и материалами нужно войти в StroyBase', 'warning')
    ...
    def inject_globals():
        ...
        if data.get('g_project'):
            ...
            data['g_buildings'] = Building.query.filter_by(project_id=data['g_project'].id).order_by(Building.name).all()
        if data.get('g_building'):
            data['g_floors'] = Floor.query.filter_by(building_id=data['g_building'].id).order_by(Floor.name).all()
        elif data.get('g_project'):
            data['g_floors'] = []
        # Дерево объектов в боковом меню (проекты с корпусами и этажами)
        data['nav_projects'] = Project.query.order_by(Project.name).all() if current_user.is_authenticated else []
        return data
