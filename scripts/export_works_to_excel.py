...
def run_export(project_id: int, output_path: str):
    ...
    with app.app_context():
        ...
        for building in buildings:
            works = Work.query.filter_by(project_id=project_id, building_id=building.id).order_by(
                Work.sort_order.asc().nullslast(), Work.id
            ).all()
            ...
            for w in works:
                ...
                by_date = {p.date: (p.daily_execution or 0) for p in progress_list if p.date}
