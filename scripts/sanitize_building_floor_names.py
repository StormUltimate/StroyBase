...
def run(
    ...
    with app.app_context():
        with DualLogger(log_path) as dual:
            ...
            log(
                f"Скрипт запущен: dry_run={dry_run}, batch_size={batch_size}, restore_prefixes={restore_prefixes}"
            )
            if dry_run:
            ...
            building_norm = {
                b.id: (b.project_id, normalized_key_for_dedup(safe_name_b(b)))
                for b in buildings
            }
            building_groups_pre = defaultdict(list)
            for bid, key in building_norm.items():
                building_groups_pre[key].append(bid)
            dup_building_count = sum(
                len(ids) - 1 for ids in building_groups_pre.values() if len(ids) > 1
            )

            floor_norm = {
                f.id: (f.building_id, normalized_key_for_dedup(safe_name_f(f)))
                for f in floors
            }
            ...
            dup_floor_count = sum(
                len(ids) - 1 for ids in floor_groups_pre.values() if len(ids) > 1
            )
            ...
            if (
                total_records > 0
                and to_delete / total_records > 0.10
                and not dry_run
                and not skip_confirm
            ):
                log(
                    f"Внимание: будет удалено {to_delete} записей (buildings: {dup_building_count}, floors: {dup_floor_count}), "
            ...
            for i, b in enumerate(buildings):
                ...
                if new_name != (b.name or ""):
                    if not quiet:
                        log(
                            f"  building id={b.id}: {repr(b.name)[:60]} -> {repr(new_name)[:60]}"
                        )
                    if not dry_run:
                        b.name = new_name
                    updated_b += 1
            ...
            for i, f in enumerate(floors):
                ...
                if new_name != (f.name or ""):
                    if not quiet:
                        log(
                            f"  floor id={f.id}: {repr(f.name)[:60]} -> {repr(new_name)[:60]}"
                        )
                    if not dry_run:
                        f.name = new_name
                    updated_f += 1
            ...
            log(
                "[3/4] Дедупликация buildings (по project_id + нормализованное имя), оставляем запись с min(id)..."
            )
            ...
            for key, ids in building_groups.items():
                if len(ids) > 1:
                    ...
                    log(
                        f"  project_id={key[0]}, name_key={key[1][:40]!r}: оставляем id={kept}, удаляем {ids[1:]}"
                    )
            for dup_id in dup_building_ids:
                kept_id = building_kept_for_dup[dup_id]
                if dry_run:
                    log(
                        f"  [DRY-RUN] Был бы удалён дубликат building id={dup_id}, ссылки переназначены на id={kept_id}"
                    )
                else:
                    Floor.query.filter(Floor.building_id == dup_id).update(
                        {Floor.building_id: kept_id}, synchronize_session=False
                    )
                    Document.query.filter(Document.building_id == dup_id).update(
                        {Document.building_id: kept_id}, synchronize_session=False
                    )
                    MaterialMovement.query.filter(
                        MaterialMovement.building_id == dup_id
                    ).update(
                        {MaterialMovement.building_id: kept_id},
                        synchronize_session=False,
                    )
                    ScheduleWork.query.filter(
                        ScheduleWork.building_id == dup_id
                    ).update(
                        {ScheduleWork.building_id: kept_id}, synchronize_session=False
                    )
                    BuildingParticipant.query.filter(
                        BuildingParticipant.building_id == dup_id
                    ).update(
                        {BuildingParticipant.building_id: kept_id},
                        synchronize_session=False,
                    )
                    Building.query.filter(Building.id == dup_id).delete()
                    log(
                        f"  Удалён дубликат building id={dup_id}, ссылки переназначены на id={kept_id}"
                    )
            if not dry_run:
                db.session.commit()
            ...
            log(
                "[4/4] Дедупликация floors (по building_id + нормализованное имя), оставляем запись с min(id)..."
            )
            if not dry_run:
                floors = Floor.query.order_by(Floor.id).all()
            ...
            for key, ids in floor_groups.items():
                if len(ids) > 1:
                    ...
                    log(
                        f"  building_id={key[0]}, name_key={key[1][:40]!r}: оставляем id={kept}, удаляем {ids[1:]}"
                    )
            for dup_id in dup_floor_ids:
                kept_id = floor_kept_for_dup[dup_id]
                if dry_run:
                    log(
                        f"  [DRY-RUN] Был бы удалён дубликат floor id={dup_id}, ссылки переназначены на id={kept_id}"
                    )
                else:
                    Document.query.filter(Document.floor_id == dup_id).update(
                        {Document.floor_id: kept_id}, synchronize_session=False
                    )
                    Plan.query.filter(Plan.floor_id == dup_id).update(
                        {Plan.floor_id: kept_id}, synchronize_session=False
                    )
                    Mark.query.filter(Mark.floor_id == dup_id).update(
                        {Mark.floor_id: kept_id}, synchronize_session=False
                    )
                    FloorMaterial.query.filter(FloorMaterial.floor_id == dup_id).update(
                        {FloorMaterial.floor_id: kept_id}, synchronize_session=False
                    )
                    FloorEquipment.query.filter(
                        FloorEquipment.floor_id == dup_id
                    ).update(
                        {FloorEquipment.floor_id: kept_id}, synchronize_session=False
                    )
                    FloorQuantity.query.filter(FloorQuantity.floor_id == dup_id).update(
                        {FloorQuantity.floor_id: kept_id}, synchronize_session=False
                    )
                    MaterialMovement.query.filter(
                        MaterialMovement.floor_id == dup_id
                    ).update(
                        {MaterialMovement.floor_id: kept_id}, synchronize_session=False
                    )
                    ScheduleWork.query.filter(ScheduleWork.floor_id == dup_id).update(
                        {ScheduleWork.floor_id: kept_id}, synchronize_session=False
                    )
                    Task.query.filter(Task.floor_id == dup_id).update(
                        {Task.floor_id: kept_id}, synchronize_session=False
                    )
                    Floor.query.filter(Floor.id == dup_id).delete()
                    log(
                        f"  Удалён дубликат floor id={dup_id}, ссылки переназначены на id={kept_id}"
                    )
            if not dry_run:
                db.session.commit()
            ...
            log("--- Итог ---")
            log(
                f"Buildings: обработано {total_b}, санитизировано названий {updated_b}, объединено/удалено дубликатов {len(dup_building_ids)}."
            )
            log(
                f"Floors: обработано {total_f}, санитизировано названий {updated_f}, объединено/удалено дубликатов {len(dup_floor_ids)}."
            )
            if dry_run:
            ...
            if not dry_run:
                ...
                bad_b = [
                    ...
                    if not _is_valid_display_name(b.name)
                ]
                bad_f = [
                    (f.id, f.name)
                    for f in Floor.query.all()
                    if not _is_valid_display_name(f.name)
                ]
                if bad_b:
                    log(
                        f"  Внимание: у {len(bad_b)} зданий название короче 3 символов или без букв/известного префикса: {bad_b[:10]}{'...' if len(bad_b) > 10 else ''}"
                    )
                if bad_f:
                    log(
                        f"  Внимание: у {len(bad_f)} этажей название короче 3 символов или без букв/известного префикса: {bad_f[:10]}{'...' if len(bad_f) > 10 else ''}"
                    )
